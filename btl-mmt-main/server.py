...
def server_signup(message):
    ...
    if row == None: #check xem có tồn tại user hay không
        cur.execute("INSERT INTO user (name, username, password, IP, status, image) values (%s, %s, %s, %s, 1, 'https://static.wikia.nocookie.net/lolesports_gamepedia_en/images/e/ef/T1_Faker_2023_WC.png/revision/latest?cb=20231021095117')", (message["name"], message["user_name"], message["password"], message["ip"]))
        con.commit()
        text = "Ok"
